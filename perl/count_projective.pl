...
while ($line = <$filef>) 
{
	...
	else
	{
		...
		@parents=(); 
		#print("newsent\n");
		$n_nonprojective = $n_nonprojective+$nonprojective;
		$n_sequence = $n_sequence+1;
	}
			
}
$ratio = $n_nonprojective/$n_sequence*100;
$key = "";
if($file =~ /UD_(.*)\//)
	{$key = $1;}
if($key eq "")
	{if($file =~ /stanford_save3\/\/(.*)_Parser\//)
		{$key = $1;}}

print "\n== $key nonprojective proportion : $n_nonprojective/$n_sequence=". sprintf("%.2f",$ratio)."%\n";
