...
class Multivocab(Configurable):
  ...
  def __call__(self, placeholder=None, moving_params=None):
    ...
    print('===multivocab.py: START Call ', ' name=', self.name, ' strategy=',self.merge_strategy)
    print('===multivocab.py: END Call ', ' name=', self.name)
    
    #additionne base_word et pretrained : base_word modifie les embeddings initiaux, les embeddings de char sont juste différents     
    if self.merge_strategy == 'concat':
      embeddings = []
      for vocab in self._vocabs:
        print(vocab.name);
      for vocab in self._vocabs:
        if vocab.name == 'words' or vocab.name == 'pretrained':
            embeddings.append(vocab(moving_params=moving_params))
      r = tf.add_n(embeddings)
      for vocab in self._vocabs:
        if vocab.name == 'chars':
            r = tf.concat([r,vocab(moving_params=moving_params)],axis=2)
      return r;
    elif self.merge_strategy == 'add':
      embeddings = [vocab(moving_params=moving_params) for vocab in self]
    elif self.merge_strategy == 'only_pretrained':
        print("===multivocab.py: POST_END Call only_pretrained strategy")
        embeddings = []
        for vocab in self._vocabs:
            if vocab.name == 'pretrained' :
                embeddings.append(vocab(moving_params=moving_params))
    elif self.merge_strategy == 'only_pretrained_and_chars':
        print("===multivocab.py: POST_END Call only_pretrained strategy")
        embeddings = []
        for vocab in self._vocabs:
            if vocab.name == 'pretrained' or vocab.name == 'chars' :
                embeddings.append(vocab(moving_params=moving_params))
    #else if merge_strategy == 'add':
  ...
  def generate_placeholder(self):
   ...
   #=============================================================
  def generate_placeholder_vs2(self):
    """"""
    
    if self.placeholder is None:
      [vocab.generate_placeholder_vs2() for vocab in self]
    return self.placeholder
