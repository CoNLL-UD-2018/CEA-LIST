...
class BaseOptimizer(Configurable):
  ...
  def learning_rate(self):
    if self.decay_steps > 0:
      lr= super(BaseOptimizer, self).learning_rate * self.decay**(self.global_step / self.decay_steps)
    else:
      lr= super(BaseOptimizer, self).learning_rate
    #CLR try
#    lr = tf.cond(tf.logical_and(tf.greater(self.global_step,10),tf.less(self.global_step,700)), 
#                  lambda: tf.cond(tf.equal(tf.floor(tf.mod(tf.floor(tf.divide(self.global_step,10)),2)),1),
#                                  lambda: 
#                                      tf.add(lr,tf.multiply(tf.mod(self.global_step,10),0.1)),
#                                  lambda: 
#                                      tf.add(lr,tf.subtract(1.0,tf.multiply(tf.mod(self.global_step,10),0.0001)))
#                                  ), 
#                  lambda: lr)
