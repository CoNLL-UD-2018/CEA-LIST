...
class RNNEmbed(BaseEmbed):
  ...
  def __call__(self, vocab, **kwargs):
    """"""
    print("===rnn_embed.py: START Call to RNNEmbed", ' name=', self.name)
    # (n x b x d)
    embeddings = super(RNNEmbed, self).__call__(vocab, **kwargs) #Genere les embeddings via BaseEmbed
    #tf.summary.histogram('embeddings', embeddings)
    # (n x b x d) -> (n x b x h)
    with tf.variable_scope('RNN'):
      recur, state = self.RNN(embeddings, self.recur_size) #Crée la cellule LSTM
    ...
    print("===rnn_embed.py: END Call to RNNEmbed", ' name=', self.name)
    return linear 
