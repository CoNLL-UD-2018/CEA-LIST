...
class BaseVocab(Configurable):
  ...
  def generate_placeholder_vs2(self):
    ...
    if self.placeholder is None:
      self.placeholder = tf.placeholder(tf.int32, shape=[None, None], name="prefix/Network/"+self.name)
    return self.placeholder
  
  #=============================================================
  def __call__(self, placeholder=None, moving_params=None):
    """"""
    print('===base_vocab.py: START Call ', ' name=', self.name, " moving_params=", moving_params)
    ...
    embeddings = self.embeddings if moving_params is None else moving_params.average(self.embeddings)
     #renvoit les embeddings qui ont pour id "placeholder[0],...placeholder[n]"
    print('===base_vocab.py: END Call ', ' name=', self.name)
  ...
  def embeddings(self, matrix):
    ...
    with tf.device('/cpu:0'):
      print('PROBLEM?',self.name.title())
      with tf.variable_scope(self.name.title()):
        print("=== base_vocab.py: SET EMBEDDINGS, name=",self.name)
