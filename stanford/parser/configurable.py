...
class Configurable(object):
  ...
  def __init__(self, *args, **kwargs):
    ...
    if not args and not is_eval:
      if not os.path.isdir(self.save_dir):
        os.makedirs(self.save_dir) #crée le dossier
      
      #réécris la configuration définitive (default+myconfig) dans le fichier du dossier de sauvegarde
  ...
  def _configure(self, *args, **kwargs):
    ...
    if args: 
      ...
      config_files = [os.path.join(os.path.abspath(os.path.join(os.path.abspath(os.path.join(__file__, os.pardir)), os.pardir)), 'config/defaults.cfg'),
  ...
  def _globlist(self, lst):
    ...
    if len(globbed) == 0:
      warnings.warn('Original list %s has no files (check the path!!)' % lst)
    return globbed
  ...
  @property
  def subpos_vocab(self):
    from parser import vocabs
    return getattr(vocabs, self.get('subpos_vocab'))
  @property
  ...
  def merge_strategy(self):
    return self.get('merge_strategy')
  ...
  def max_train_time(self):
    return self.getint('max_train_time')
  @property
  def min_percent_per_hour(self):
    return self.getfloat('min_percent_per_hour')
  @property
