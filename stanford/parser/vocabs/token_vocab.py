...
class TokenVocab(BaseVocab):
  ...
  def __init__(self, *args, **kwargs):
    ...
    print ("===token_vocab.py: Start init, recount:%s, self.filename:%s"%(recount, self.filename))
    if recount:
      self.count()
    else:
      print ("===token_vocab.py: os.path.isfile(self.filename):",os.path.isfile(self.filename))
  ...
  def count(self, conll_files=None):
    ...
    print ("token_vocab, conll_files: ", conll_files)
        ...
        for line_num, line in enumerate(f):
          try:
            ...
            if line and not line.startswith('#'):
              ...
              if not self.cased:
                token = token.lower()
              #rel doesn't read subtype rel (specific to language)
              if self._conll_idx == 7:
                  token = token.split(':')[0]
              self.counts[token] += 1
          except:
            print("line is [",line,"]")
...
if __name__ == '__main__':
  ...
  from parser.vocabs import PretrainedVocab, TokenVocab, WordVocab, TagRepVocab
